<!-- 完整功能說明（简体中文）。README 於 2026-07-26 精簡為產品主頁，此處保留每個 tab 的詳細走查、Kimi 報告對標與全部截圖。 -->
...
这是一个**免费开源**的个人项目。如果它帮你省了事、或让你看球多了点乐子，欢迎请作者喝杯咖啡（也可在网页右上角点「**♥ 赞赏支持**」扫码）。

<p align="center">
  <img src="../data/sponsor.png" alt="赞赏码（支付宝 / 微信）" width="540">
</p>

> **赞赏纯属自愿，不解锁任何功能、不构成购买任何预测服务**——所有功能对所有人永久免费。自部署时把你自己的收款码图片替换 `data/sponsor.png` 即可。
...
<p align="center">
  <img src="./screenshot-dashboard.png" alt="赛事看板：正在比赛 / 即将开赛 / 已结束" width="820">
...
<p align="center">
  <img src="./screenshot-dashboard.png" alt="Codex / Claude skill 打开的世界杯比分预测器赛事看板" width="820">
...
| 🔍 **机制解读** | 任选对阵出一张**只读、描述性**的市场机制卡：Shin 去水真实隐含概率 + 抽水（水位结构）、模型 vs 市场分歧地图（KL + 逐结果差）、赛前盘口移动——每条分歧**强制挂「市场对、模型错」先验**（本模型 CLV 已证 <0）。只陈述认知，**从构造上就发不出任何买入/跳过指令**（违规词守卫 + 渲染自检） |
| 📒 **竞彩复盘** | 长期**手动复盘闭环**：赛前录入真实竞彩让球线 + 你自己的判断（模型预测录入即冻结），赛后手填 90 分钟比分，得到**单场三方对账**——你 / 模型 / 市场各自对错。它存在的意义是直面一个事实：你和模型长期都打不赢市场；**从 schema 层就禁止聚合成下注信号**（数据结构里不存在 ROI / 胜率字段） |
| 📱 **移动端优先看板** | 整个 UI 在 ≤430px 依然成立：三列紧凑赛程行、比分单行不换行、每场即将开赛带**三段式胜平负概率条**、弹窗纵向堆叠——手机上零横向滚动 |
...
**网页八大 Tab——统一图标、并按「核心 / 对照」两组划分——把整个"预测期"做成了一个能玩的实时产品：**
...
- **每一行赛程都是发射台**：除「看预测」与「深度报告」外，每场还带一键直达**机制解读**与**竞彩复盘**的按钮（对阵与日期自动预填，不用跨 tab 重输队名）。淘汰赛行与日期头带**阶段徽章**（1/8 决赛 / 1/4 决赛…），即将开赛区顶部交叉链接到晋级树（"这场的胜者去哪、对位谁"）。
- **移动端优先**：≤430px 下看板重排为紧凑三列行，每场即将开赛下方带**三段式胜平负概率条**（点开可看精确百分比），比分单行、弹窗纵向堆叠——在球场看台上单手可用。
<p align="center">
  <img src="./screenshot-mobile.png" alt="375px 手机端看板：LIVE 实时胜平负条、每场三段概率条、阶段徽章与一键直达入口" width="300">
  <br><sub><em>同一个看板在手机上（375px）—— LIVE 实时条、逐场概率条、阶段徽章、一键直达解读/复盘。</em></sub>
</p>
...
<p align="center">
  <img src="./screenshot-scoreboard.png" alt="正确比分概率盘：按玩法盘面逐档给出模型概率，大比分自然浮现并高亮最可能比分" width="760">
  <br><sub><em>正确比分概率盘——固定盘面逐档概率 + 高亮最可能比分 + 大小球 / 任一方≥3 / 双方进球 + 比分矩阵热力图。</em></sub>
</p>

#### 📣 比赛解读（把概率翻成球迷语言）
在「看预测」弹窗与「对阵分析」报告头各置顶一张**绿色解读卡**，把模型干巴巴的胜率翻成一句**球迷语言**：球队昵称（桑巴军团 / 高卢雄鸡…）、大热 / 占优 / 势均 / 爆冷的叙事、**本场动态竞彩让球线**（让N 或平手倾向，同样翻成球迷语言，并明确标注「**模型定线，非官方盘**」），以及进球大战 / 闷战的氛围预判。它**纯只读、绝不碰预测引擎**——只是把模型已经算出的数字换成人话。**合规由设计保证**：内置违规词守卫（与机制解读共用 **73 词并集** denylist），禁"稳赚 / 必中 / 包赢"等担保或诱导下注词（生成器本就不产出，再加一道运行时守卫，违反即抛）；每条文案必带「📌 仅为模型概率推演，非投注建议，理性观赛、量力而行」尾注；只陈述概率与事实，**不导购彩票、不给出票/买入建议**。
<p align="center">
  <img src="./screenshot-narrative.png" alt="比赛解读卡：把模型胜率翻成一句球迷语言，附动态让球倾向与「非投注建议」声明" width="760">
...
<p align="center">
  <img src="./screenshot-update.png" alt="右上角仓库更新检测：落后远程 main 的提交数 + 发现新版本提示弹窗" width="760">
  <br><sub><em>仓库更新检测——右上角显示本地落后远程的提交数，发现新版本时提示"对 Claude 一句话更新"。</em></sub>
</p>

#### ⚡ In-play 实时胜平负（差异化护城河）
比赛进行中，看板的 LIVE 卡片显示一条实时胜平负堆叠条：赛前 Dixon‑Coles 的期望进球 λ 按**剩余时间缩放**，叠加**当前比分**做泊松卷积，得"从现状到终场"的主胜/平/客胜——随每一个进球和分钟跳动。东道主场次与**赛前冻结预测同口径接入 host + 环境修正**（如美国主场：开球时刻胜率 中立 30.1% → 主场修正 37.3%），实时条与冻结账本在 t=0 永不打架。**只读引擎、严格隔离，绝不污染赛前预测的可证伪性。**
...
<p align="center"><img src="./screenshot-manager.png" alt="对阵分析深度报告" width="760"></p>
...
<p align="center"><img src="./screenshot-lineup.png" alt="赛前(纯模型) vs 首发确认 概率对比 + 逐人可用性" width="760"></p>
...
<p align="center"><img src="./screenshot-bracket.png" alt="2026 官方赛制括号投影" width="820"></p>
...
<p align="center"><img src="./screenshot-champ.png" alt="夺冠概率可信区间与贝叶斯实力榜" width="760"></p>
...
<p align="center">
  <img src="./screenshot-market.png" alt="市场对标 / CLV 诚实门槛逐条清单：样本量 / 平均 CLV / t 值 / 跑赢闭盘比例 + 是否达标" width="760">
  <br><sub><em>CLV 诚实门槛逐条清单——真实数字透明展示，每条标注是否达标，价值面板锁着的原因一目了然。</em></sub>
</p>

#### 🔍 机制解读（描述性市场认知——绝不是指令）
任选对阵（或从看板一键直达）出一张**只读市场机制卡**：**A 水位结构**——Shin 去水后的真实隐含概率、庄家抽水（overround）与水位异常标注；**C 模型 vs 市场分歧地图**——KL 散度 + 逐结果概率差，且**每条分歧都强制配「市场对、模型错」先验横幅**（本模型样本外 CLV 已证显著 <0、让球点估 MAE 打不赢市场闭盘线——分歧更可能是模型误差，而非市场错价，这句话用琥珀色写在每张卡上）；外加**赛前线移动**（开盘→闭盘隐含概率位移）与让球盘的同款 2 路 Shin 去水。「诱惑形态」段（大热必死/FLB 命名）**冻结在统计闸门后**——按盘种分桶，n ≥ 30 且置信区间不跨 0 才解锁，达标前根本不渲染。**从构造上它就发不出任何买入/跳过/价值指令**：违规词 denylist + 渲染时自检双重守卫（违反即抛异常，测试套件为每个渲染分支钉了反例）。
<p align="center"><img src="./screenshot-explainer.png" alt="机制解读卡：Shin 去水水位结构、模型 vs 市场分歧（强制「市场对模型错」先验横幅）、赛前线移动" width="760"></p>

#### 📒 竞彩复盘（长期诚实镜子，不是荐单）
一个围绕竞彩让球的**手动单场复盘闭环**：赛前录入真实竞彩让球线 + 双边 cover 赔率 + **你自己的判断**（看好让球方 / 看好受让方 / 跳过，可附一句理由）——模型的三向概率与最可能比分在**录入那一刻冻结**。赛后手填 **90 分钟比分**（界面会提醒：竞彩按 90 分钟结算，加时点球只决定晋级、不进本盘），tab 自动做**单场三方对账：你 / 模型 / 市场**，各自判 对 / 错 / 走盘（走盘全员 void 不计；你选「跳过」则你那行不计分）。派生的**读盘卡**把让球线翻成带真实队名的大白话（"加拿大(-1) 要赢 2 球以上；恰好赢 1 球走盘退回"）——纯规则翻译、零方向。这个 tab 存在的意义，是让一个事实随时间可见：**你的判断和这个模型，长期都打不赢市场闭盘线**——并且它**从 schema 层就被禁止变成荐单引擎**：数据结构里不存在 ROI / 盈亏 / 任何"率"字段，跨场聚合成行动信号在设计层直接拒绝，测试套件断言这些字段永不出现。
<p align="center"><img src="./screenshot-jcreview.png" alt="竞彩复盘 tab：录入真实竞彩让球线与自己的判断，模型预测即刻冻结，赛后填 90 分钟比分做你/模型/市场三方对账" width="760"></p>
...
<p align="center"><img src="./screenshot-xuanxue.png" alt="玄学占卜：七法 vs 诚实基线，覆盖本届与 4.9 万场历史回测" width="760"></p>
...
| **胜平负命中率** | **59.7%** | 三向 argmax 准确率——**全部国际赛混合口径**（含大量预选赛/友谊赛）。仅世界杯正赛（as-of，2014/18/22/26 合并 n=295，`bt_wc.py`）：RPS 0.186 / 命中 61.0% CI[55.6, 66.8]——其中经典 64 队时代各届仅 53–59%（RPS≈0.20）；2026 同场闭盘赔率胜过模型（RPS 0.1462 vs 0.1514）。引用请用分层数字，勿用混合口径 |
...
<p align="center">
  <img src="./screenshot-verify.png" alt="本届实测验证：36 场、赛果命中 55.6% 正趋近 ~59.5% 回测真值，含置信度分桶与失手归因" width="820">
...
> 📓 比赛日运行看一页纸 **[比赛日运行手册](./RUNBOOK.zh-CN.md)**。
...
├── test_core.py   回归测试（140+ 项且持续增长——以 `pytest test_core.py -q` 输出为准：预测合理性 / 矩阵归一 / 2026 官方同分规则 / 缓存新鲜度 / API 冒烟 / 红线守卫 / 并发安全 / …）
