...
## 2026-08-03（二十四轮）首页比赛流出概率 + 点场次把对阵带进分析

### 触发
用户报两点：① 赛事总览看不到比赛场次预测结果；② 点场次跳转对阵分析后
「两个球队的输入框获取不到 id」。两条都复现了，都是真缺陷。

### ① 首页无预测：设计撞上现实，但不放宽铁律
首页铁律③「未来比赛概率只认冻结账本、绝不现算」是为了防同一场两个数字；而俱乐部冻结
被开球时区闸挡着（0/5 未核验）→ 每行都是「预测待冻结」，用户看到的就是一片空。
**修法不是放宽铁律**：
- 后端 `/api/home` **逐字不动**（仍只给冻结值，`test_home_unfrozen_fixture_contains_no_probability` 仍锁）。
- 前端新增 `hmFillEstimates()`：渲染后对未冻结行向 **看板端点** `evGet(ev,'overview')` 取估算
  补位。与看板逐位同源 → 仍然不会出现两个数字；首页后端也仍然不训练不现算（铁律①）。
- 标注沿用看板文案「当前模型估算（未冻结）」/「升班马合训估算（未冻结）」；
  取不到（无模型/不在窗内/端点失败）就保持「预测待冻结」，不猜不填。

### ② 点场次不带队：路由丢参 + 一段漏声明形参
- 首页行原来只跳 `#<event>/matchup`，落地后由 `ranking[0]/[1]` 预填 → **点哪场都出实力榜
  前两名**。改成带 `?h=&a=`，且**同步预填**（不等 overview 回来，否则会把用户点的那场覆写）。
- **qs 必须四段全通**：`hmGo → hashchange → selectEvent → renderEventView → evShowTab`，
  外加首次加载的深链路径 `selectEvent(ev, h||'', qs)`——漏了最后这条＝站内点击能用、
  刷新和分享链接不能用，是最难自查的那类坑。
- ⚠️ **施工时漏了 `renderEventView(key, tab, qs)` 的第三个形参**：源码里三处 qs 齐全、
  grep 全绿，运行时 ReferenceError → **整个赛事视图不渲染**，`#ev_home`/`#ev_away` 根本不存在
  ——这正是用户说的「获取不到 id」。是 CDP 实测抓出来的，不是测试抓出来的。
- 顺带：看板「未来赛程预测」整行可点（`tr.ev-clk`），同一条 `hmGo` 路径；带撇号队名
  （Nott'm Forest）用 data-* 属性 + `hmAttr()` 转义，不进内联 onclick 字符串。
- 深链回写：URL 里带 football-data 英文名（短、可分享），预测成功后把输入框回写成中文显示名。

### 验证
- **新脚本 `scripts/ui_click_check.py`**（真实浏览器 CDP，六项断言，退出码即结论）：
  首页有估算且无裸数字 / 点首页场次 hash 带参且输入框预填出卡 / 看板整行可点（专挑带撇号
  的队名点）/ 深链直开同样预填 / 世界杯页零回归。**实跑 exit=0，六项全过**，
  产物 `docs/evidence/ui-click-check.json`。
  立这个脚本的理由写在文件头：pytest 只能 grep 到源码写了 `?h=&a=`，**写了不等于传到了**。
- `pytest test_core.py -q` **228 passed**（新增 2 个签名护栏：qs 四段签名齐全、
  首页估算必须走看板端点且带口径标注）。
- 截图 `docs/evidence/home-match-stream-estimates.png`。

### 遗留
clubverify 冻结链路仍未接升班马模型、开球时区核验 0/5，08-21 首轮前一起解决。
