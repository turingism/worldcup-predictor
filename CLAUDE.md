...
## 📌 最新接手（2026-08-03 二十四 · 首页比赛流出概率 + 点场次带对阵进分析，228 测试绿）
- **用户报两点，都是真缺陷**：① 赛事总览看不到任何预测结果；② 点场次跳到对阵分析后
  「两个球队的输入框获取不到 id」。
- **① 根因不是 bug 是设计撞上现实**：首页铁律「未来比赛概率只认冻结账本、绝不现算」
  （防同一场两个数字），而俱乐部冻结被时区闸挡着 → 每行都是「预测待冻结」。
  **修法不是放宽铁律**：后端 `/api/home` 逐字不动（仍只给冻结值，测试仍锁），
  改由**前端**向**看板端点**取估算补位（`hmFillEstimates()` → `evGet(ev,'overview')`）——
  与看板逐位同源，所以仍然不会出现两个数字；标注沿用看板文案
  「当前模型估算（未冻结）」/「升班马合训估算（未冻结）」；取不到就保持「预测待冻结」。
- **② 根因=路由丢参 + 一段没声明形参**：首页行只跳 `#<event>/matchup`，落地后由
  `ranking[0]/[1]` 预填 → 点哪场都出实力榜前两名。改成带 `?h=&a=` 并**同步预填**
  （不等 overview，否则回来会把用户点的那场覆写）。**qs 必须四段全通**：
  `hmGo → hashchange → selectEvent → renderEventView → evShowTab`，
  外加首次加载的深链路径 `selectEvent(ev,h||'',qs)`（漏了它=站内点能用、分享链接不能用）。
  ⚠️ 施工时漏声明 `renderEventView(key, tab, qs)` 的第三个形参 → 运行时 ReferenceError →
  **整个赛事视图不渲染**，输入框根本不存在，正是用户说的「获取不到 id」。
- **顺带**：看板「未来赛程预测」整行可点（`tr.ev-clk`），同一条 `hmGo` 路径；
  带撇号队名（Nott'm Forest）走 data-* 属性 + `hmAttr()` 转义，不进内联 onclick 字符串。
- **新验收脚本 `scripts/ui_click_check.py`**（真实浏览器 CDP，六项断言，退出码即结论）：
  pytest 只能 grep 到源码写了 `?h=&a=`，**写了不等于传到了**——本轮缺陷 grep 全绿却整页不渲染。
  行为事实一律以该脚本为准；pytest 侧只留签名护栏。
- **遗留不变**：clubverify 冻结链路仍未接升班马模型、开球时区核验 0/5，08-21 首轮前一起解决。
  冻结一旦启用，首页与看板会自动从「估算」切回「赛前冻结」（两处都是冻结值优先）。

## 📌 上一次接手（2026-08-03 二十三 · 升班马通道推广到五大联赛，逐联赛权重，226 测试绿）
- **触发**：用户报「西甲、法甲、德甲里还是有暂无数据」。二十二轮的边界（通道仅英格兰）
  是**机制限制不是数据限制**——`comp_tier` 把西乙/意乙/德乙/法乙与各自顶级判成同一 tier，
  comp_weights 分不开；英格兰能分只因 "championship" 关键词撞车。
- **机制换掉撞车**：`data.build_training_frame` 的 comp_weights 查表改为
  **赛事名精确键 > tier > 1.0**（tier 键既有行为逐字节不变，国家队侧零影响）。
  改后 E0 复跑**数字逐位复现**（w=0.25→0.1919，网格 0.1931/0.1941/0.1949），
  证明精确键与旧撞车键等价——这是这次机制替换的验收依据。
- **逐联赛闸门（bt_promoted.py 参数化五联赛，裁决落 backtest.md 第九-2 节）**：
  **五家全过**。最优 w：E0 0.25 / SP1 0.25 / **I1 1.0** / **D1 1.0** / F1 0.25。
  **⚠️「降权更好」不是普适结论**——意德满权更优、方向与英西法相反，但网格跨度仅
  0.0009/0.0012（E0 是 0.0030）属弱确定。**勿把英超的 0.25 往别处抄**（同 hl 逐联赛纪律）。
- **接线泛化**：`promoted_newcomers(code)` / `get_promoted_model(code)` 按 `clubdata.FEEDER`
  推导，缓存 `model_<code>promo.pkl`，权重键=feeder 赛事名；`clubpredict.PROMOTED_FEEDER_W`
  取代 `PROMOTED_E1_W`。app 三个消费方与 CLI 去掉 E0 硬编码；响应字段
  `promoted_e1_weight` → `promoted_feeder_weight` + `promoted_feeder`（中文简称）。
- **实测结果：五大联赛未来赛程卡片 no_model 归零**。当前升班马新面孔——英超 Coventry/Hull、
  西甲 La Coruna/Malaga/Santander、德甲 Elversberg、法甲 Le Mans；意甲本季无新面孔
  （升班队近 7 季都有意甲样本，标准模型本就覆盖）。
- **遗留不变**：clubverify 冻结链路仍未接升班马模型（账本里这些场次仍会 no_model），
  与开球时区核验 0/5 一起在 08-21 首轮前解决。

## 📌 上一次接手（2026-08-03 二十二 · 升班马合训模型接进 Web 三个消费方，224 测试绿）
- **背景**：二十一轮把 E1 降权 w=0.25 合训采纳了，但**只接在 clubpredict CLI 层**——
  网页侧看板/单场/竞彩三处仍全部 `no_model`（实测 `/api/club/predict?home=阿森纳&away=考文垂`
  返回「找不到球队」）。本轮把它接完。
- **共用入口，零第二套**：app 新增 `_promoted_newcomers(code)`（按 E0/E1 CSV + ESPN 赛程缓存
  mtime 记忆化，异常一律退化空集=通道关闭，绝不拖垮看板）与 `_club_model_for(code, teams, promoted)`
  → 三个消费方（`/api/club/overview` upcoming、`/api/club/predict`、`/api/jc_review` club 分支）
  全部走这一对函数 + `clubpredict.resolve_promoted` 兜底解析。实测三处数字逐位同源。
- **口径逐行标出（本轮的硬要求）**：升班马场次与纯英超场次不是一个模型，同表并列不作声就是
  混口径。响应带 `basis` ∈ {league, promoted_cotrained} + `promoted_e1_weight` + `promoted_note`；
  前端看板逐行显示「升班马合训估算（英冠权重 0.25）」，单场卡与竞彩预览加脚注。
- **对阵分析过程数据帧同步并英冠**：升班马在英超帧里零场次，近况/主客拆分/交锋会整片空白；
  概率既已用合训模型，`facts` 帧也并上 E1，并在 `strength_note` 写明「本场过程数据帧含英冠场次」。
- **实测**：英超首轮 10 场全部出数（原 2 场 no_model）——阿森纳 82/13/6 对考文垂、
  赫尔城 13/19/69 对曼联；纯英超场次逐位不变（埃弗顿 0.4038 前后一致）。
- **⚠️ 其余四联赛的升班马仍 no_model 且这是设计**：通道靠 `comp_tier` 关键词撞车才能把英冠降权，
  西乙/意乙/法乙与其顶级同 tier 不可分（见二十一轮条）。当前空缺：西甲 Santander、
  德甲 Elversberg、法甲 Le Mans。要覆盖须先给 comp_weights 一个不依赖撞车的按联赛通道。
- **遗留不变**：clubverify 冻结链路仍未接升班马模型（账本里这些场次仍会 no_model），
  与开球时区核验 0/5 一起在 08-21 首轮前解决。

## 📌 上一次接手（2026-08-03 二十一 · 升班马单场路径：E1 降权 w=0.25 采纳，220 测试绿）
- **触发**：用户竞彩截图=26-27 英超首轮 阿森纳 vs 考文垂（8-22 北京 03:00）/ 赫尔城 vs 曼联
  （8-22 19:30），两升班马近 7 季无 E0 样本 → clubpredict 拒绝出数。**07-08 否决的是 E1 全量
  (w=1) 并入一般单场**，预留的「E1 降权(w<1)并入」研究本轮做完并采纳。
- **`bt_promoted.py` 专项闸门（裁决落 docs/backtest.md 第九节）**：5 赛季初 cutoff × 537 场
  涉升班马留出，**w=0.25 RPS 0.1919 最优且权重单调劣化到 w=1.0=0.1949**（与全量否决同向互证）；
  5/5 cutoff 优于均匀基线 0.2409；对 B365 闭盘差 +0.0176≈模型对市场常态差距。基线现状=35% 场次
  完全拒绝、其余用数年前陈旧参数。
- **接线（clubpredict.py，纯 CLI 层，零碰既有模型路径）**：`PROMOTED_E1_W=0.25`、
  `promoted_newcomers()`（当季 E0 赛程(clubfixtures 缓存) ∧ 近7季无E0 ∧ 有E1 → 现=Coventry/Hull）、
  `resolve_promoted()` 解析兜底、`get_promoted_model()`（E0+E1 合训，缓存 model_E0promo.pkl，
  schema/hl/权重/双 CSV mtime 四重守卫）。**只在涉升班马新面孔时启用**，一般场次纯 E0 零改动。
  实测：阿森纳 81.7/12.7/5.5 最可能 2-0；赫尔城 vs 曼联 12.5/18.7/68.8 最可能 0-2，带口径标注。
- **⚠️ E1 降权通道=comp_weights 的关键词撞车巧合**：`data.comp_tier("English Championship")=="major"`
  （本意欧锦赛）/EPL=="other" 恰好可分——护栏测试 `test_comp_tier_championship_collision_locked`
  锁死，动 comp_tier 前先看它。**此路仅适用英格兰**（西乙/意乙与其顶级同 tier 不可分）。
- **遗留**：clubverify 冻结链路未接升班马模型（这些场次账本仍 no_model，须单独一轮，与开球
  时区核验 0/5 一起在 08-21 首轮前解决）；E1 26-27 CSV 落地后合训缓存随 mtime 自动重训。

## 📌 上一次接手（2026-08-03 二十 · 联赛赛程接 ESPN 主源，218 测试绿）
...
- **P0-H 首页总览（79e2f8a）**：新增 `home_dashboard.py` + `/api/home` + `#home` 默认落地。
  ...
  **验证账本按赛事并列，响应根节点禁止 total/summary/overall_accuracy**（隔离不变量的 API 延伸）；
  jc_review 整体不进首页。世界杯数字复用 `verify.evaluate`（AST 核过只读），与 `/api/verify` 同源同值。
  **不冒充**：seasonsim 现为 season=2025-26/mode=retro，只有赛季与 mode 都匹配才给 title_favorite
  （否则就是拿上季终局 100% 冒充新季预测——Codex 抓到的真坑）。未来比赛概率只认冻结账本，
  未冻结显示「预测待冻结」；联赛看板同场改冻结值优先并标口径，消除一场两数字。
  实测冷构建 181ms / 热缓存 1ms / JSON 9.4KB。
- **用户报的三点已修（821ffae）**：① 点击不跳转=**页面从来没有 hashchange 监听**，路由只在加载时
  跑一次，首页 `location.hash=…` 只改地址栏；补路由后前进/后退也通了。② 首页隐藏菜单是我与 Codex
  收口时定错的，现首页保留菜单、「赛事总览」为首项且走 push（后退能回刚才的赛事），移动端 sticky 吸左。
  ③ 菜单重做：按 universe 分组 + 状态圆点 + 等宽数字 + 选中左强调条。
- **刷新闪世界杯页面已修（本轮）**：根因=静态 HTML 本身就是世界杯页（title/页头/8 Tab/`#verify`
  默认可见），而首页落地要等 `/api/events` 返回才切换，**闪动窗口=一次网络往返**。修法（与 Codex 收口）：
  `<head>` 内联同步 boot 脚本按 hash 打 `boot-home/event/wc` 类（key/别名/默认赛事由服务端注入，
  代码零赛事 key 字面量）→ CSS 首帧压住 `.tabs`/`#verify`（**`#verify` 有内联 display:block，
  必须 `!important`**）→ 视图切换完成后 `finishBoot()` **清除 boot 类**（不清除的话之后进世界杯页
  Tab 会被永久压住，Codex 抓到）→ 首页渲染不再等 events。页头原值在改写前存进 `window.__HDR_DEF`，
  否则 `_HDR_DEF` 会捕获到首页文案、切回世界杯串标题。
- **验收工具**：新增 `scripts/ui_check.py`（headless Chrome + CDP 实测盒模型；`--boot` 跑首帧与
  路由生命周期）。它抓到并修掉真缺陷：世界杯验证账本 7 列网格在 431–1000px 撑破页面。
  **pytest 只做静态护栏**（CSS 规则没被删），布局事实一律以该脚本退出码与 JSON 为准——
  「关 JS 验 boot 类」这种测试逻辑上不成立（关了 JS boot 脚本也不执行），已被 Codex 指出并改掉。
- **CSS 层叠暗坑**：新样式追加在长样式表末尾会覆盖更靠前的媒体查询（实测把移动端胶囊背景冲成透明）。
  布局类规则要限定 `@media(min-width:761px)`，或插到媒体查询之前。
- 遗留：有赛程模式的比赛流未实测（26-27 赛程未发布，未造合成图冒充）；P0-B~P2 未动。

## 📌 上一次接手（2026-07-25 十八 · events 更名 + P0-A 俱乐部冻结/结算链路，186 测试绿）
...
## 📌 上一次接手（2026-07-23 十五 · E2 欧战账本完成：五季 1552 场落统一模型，150 测试绿）
- **E2 完成**：eurodata.py（月窗回收+合并式落盘防覆盖丢场、映射在装载层）——21-22~25-26 五季欧冠+欧联 **1552 场/195 对 tie/10 决赛**，欧冠覆盖精确（125×3+189×2）；tie_id 两回合配对+决赛 neutral=True+ESPN_FIX 约 70 条映射。已知限制落档：**ESPN 比分为终局口径（含加时）**，E3 校准只用小组/联赛阶段场次（纯 90 分钟）规避。150 全绿、golden diff 干净、data/euro/ 入 git。
- **下一步：E3 跨联赛强度校准**——新回测脚本（bt_crossleague.py 之类）：① 取欧战小组/联赛阶段中「双方均属五大联赛」的交锋（eurodata.load() 过滤 leg==0 且非决赛 + 双方在 clubpredict._league_teams 池）；② 方法：各联赛独立模型给出两队攻防参数 → 联赛间加常数位移 δ_league（一联赛定 0 做基准）拟合欧战交锋赛果（泊松似然或简化为净胜球回归）；③ 检验：留出季/交叉验证看校准后欧战对阵预测 RPS 是否显著优于「无位移直接比」基线——**不显著则如实报告并采保守方案（维持诚实拒绝跨联赛或仅展示带宽提示）**；④ 方法+数字落 docs/backtest.md 新章节。样本量注意：五季小组阶段五大互相交锋估计仅几十到百余场，可能不显著——这正是「校准不显著如实报告」条款预设的场景，勿强行显著。
- 长期跟踪：_CUR_END+1（8 月）/ launchd 明晨观测 / events 更名 / 26-27 名单。

## 📌 上一次接手（2026-07-23 十四 · E1 欧战选型实测完成：ESPN 主源，149 测试绿）
- **E1 完成（裁决落 data-sources.md 第八节）**：ESPN uefa.champions/uefa.europa 六探针实测（18-19 至 24-25 决赛比分逐一与史实一致，24-25 联赛阶段/淘汰赛日有数据），**leg 字段+notes 合计比分原生**，26-27 资格赛窗 0 场（新鲜度留观测）；openfootball 备源（文本格式/对齐成本/无结构化 leg 落选主源）。
- **下一步：E2 欧战账本**——新模块（如 eurodata.py）：① 回收器：ESPN 日期窗迭代（每季约 15-17 比赛周，按周窗分块非逐日，复用 live._fetch_json）拉近 3-5 季欧冠+欧联完场 → ② 统一 match 模型（date/home/away/score/neutral=决赛 True 其余 False/tournament="UEFA Champions League"|"UEFA Europa League"）+ 欧战特有列（leg、tie_id 或 agg 备注）→ ③ 队名映射：ESPN 显示名→football-data 拼写（五大联赛队复用 CLUB 键；非五大队新增，中文映射进 _CLUB_SRC 或独立段待定）→ 缓存 data/euro/ + 测试（回收样本量/两回合配对/映射覆盖）。注意：欧战账本不进五联赛模型训练帧（E3 才做校准），先独立成库。
- 长期跟踪：_CUR_END+1（8 月）/ launchd 明晨观测 / events 更名 / 26-27 名单。

## 📌 上一次接手（2026-07-23 十三 · **阶段 B 实体层统一表全清**，149 测试绿）
...
## 📌 上一次接手（2026-07-23 十一 · D2 每日抓取完成（两次实跑验收），147 测试绿）
...
## 📌 上一次接手（2026-07-23 八 · C6 机制解读完成，145 测试绿）
- **C6 完成**：联赛机制解读 Tab=`evExplainCards` 参数化组件（方法论正文四段+七维联赛/世界杯差异表，独立拟合/联赛内相对值/neutral=False/hl=365 全覆盖，一份实现五联赛共用）。「共用世界杯正文」落地口径已记 progress 第八轮（wc 机制解读 Tab 是赔率交互工具且需 golden 不变，故正文为联赛侧新建、内容与档案一致、wc 零触碰）。145 全绿、golden diff 干净、c6-ligue1-explain.png 入库。
- **下一步：C7 竞彩复盘联赛入口**（阶段 C 收官项）：扩展方式接入——jc_review.store_path(event) 已按注册表隔离（P1-③），后端 /api/jc_review 应已支持 ?event=（验证之），联赛 jc Tab 复用世界杯竞彩复盘前端表单（红线 2：不改 jc_review.py 世界杯逻辑；红线最严区：无率、无聚合、90 分钟手填）。完成后阶段 C 全清，转阶段 D 运维自动化（8 月初前：每日抓取脚本+fixtures 未来赛程）。

## 📌 上一次接手（2026-07-23 七 · C5 市场对标完成，145 测试绿）
- **C5 完成**：club_market.py（25-26 整季两段 as-of 防泄漏、Shin 去水、三方同场，口径=bt_club_market）→ market_<code>.json → `/api/club/market` → 市场对标 Tab（三方 RPS/命中对比+诚实层文案+赛季末周逐场对照）。实测闭盘全联赛 ≤ 模型（E0 .2028 vs .2090），与档案一致；145 全绿、golden diff 干净、c5-laliga-market.png 入库。**wc 市场 Tab 截图未验证**（首载拉 ESPN 外网，headless 无代理挂起——既有环境限制；已以 git diff 零改动+golden diff 替代，见 progress 第七轮）。
- **截图模式固化**：macOS 无 timeout；headless Chrome 用「后台起 + for 循环 watchdog kill」；改 templates 后必须重启实例（debug=False 不重载模板）。
- **下一步：C6 机制解读**（共用世界杯正文 + 联赛差异章节：每联赛独立拟合/联赛内相对值/neutral=False/hl=365。世界杯机制解读正文在 index.html explainer 区，找到其渲染源，联赛 Tab 以参数化方式复用正文+追加差异章节，勿复制正文）。后续 C7 jc 联赛入口 → 阶段 D 运维（8 月初前，含每日抓取）。

## 📌 上一次接手（2026-07-23 六 · C4 夺冠概率页完成，144 测试绿）
...
## 📌 上一次接手（2026-07-23 三 · 阶段 C 开工：C1 看板补齐 + 归档观测 + 生产重启，142 测试绿）
- **用户指令直启阶段 C（功能同步七项）**，阶段 B 顺延（欧战前仍须做）。**C1 完成**：clubsim.standings()（final_table 改名次投影，行为不变）+ overview API 补 standings/latest_matchday + 前端两张参数化卡片（最近完赛轮/赛季积分榜，冠军/降级着色，显式空态）。验收：golden diff 五端点逐字节一致（stash 前后双捕获）、evidence 两截图（c1-epl-dashboard / c1-wc2026-dashboard-regression）、142 全绿。
- **遗留清零**：wc2026 归档模式实测激活（回顾模式+轮询停，账本终局 104/104、胜平负 67.3%、决赛西班牙 1-0 阿根廷比分+赛果全中）；**生产实例已 kickstart 重启**（按「决赛回补后重启」既定计划），07-19 深夜大修+C1 全部生效（实测 status=archived、standings 在位）。
- **下一步：C2 对阵分析**（比分概率分布/胜平负/球队数据展开区——近 6 轮/交锋/主客场拆分/攻防强度「联赛内相对值」，manager.py 共用函数已裁决直用，展开区带数据截止时间，缺数据显「暂无数据」）。后续 C3 赛季推演（禁树状图）→ C4 夺冠概率 → C5 市场对标 → C6 机制解读 → C7 jc 联赛入口。events「25-26」更名仍待用户拍板。

## 📌 上一次接手（2026-07-23 二 · P2 清单替换落档 + A3 升班马映射修复，阶段 A 收口，142 测试绿）
- **P2 清单已被用户指令替换为「功能同步七项」**（看板补齐→对阵分析→赛季推演→夺冠概率→市场对标→机制解读→jc 联赛入口），权威版+施工纪律见 progress.md 07-23 第二轮条；旧 P2 项（teams 统一表/欧战/每日抓取/fixtures）归位任务书阶段 B/D/E。**红线勘误（用户裁决）**：禁 emoji 范围=正文与数据区文案，Tab 栏及功能图标位除外。
- **A3 缺口实测修复（红线 5 冲突落档）**：第五轮「映射零缺口」只对五大联赛帧成立；feeder 25-26 终表前三（26-27 升班马候选）六队无映射——teams_zh.CLUB +6（考文垂/米尔沃尔/桑坦德竞技/拉科鲁尼亚/埃弗斯贝格/勒芒），新增 test_teams_zh_promoted_candidates_mapped 锁定，**141→142 全绿**。
- **阶段 A 宣告收口**（A1 回补/A2 重训/A3 升班马+映射/A4 口径勘误段全部核对✓）。**下一步：阶段 B**——B1 评估（第四/五轮已有档案：物理单表 M 级、用户已裁决「做」）；B2 teams 统一表（336 国家队+144 俱乐部+universe 字段+双语，俱乐部池共享/国家队池隔离）；B3 共用实现已裁决=manager.py 函数（两宇宙实测过，勿造第二套）。B1 保险丝：动世界杯账本结构必须暂停等拍板。
- 遗留：wc2026 归档模式+生产实例重启未观测；附加赛升班马名单待官方确认（club_preseason 运行闸维持）。

## 📌 上一次接手（2026-07-23 · 收官任务书第一轮：基线修复，141 测试绿）
- **开轮基线红一项**：`test_freeze_ledger_records_adjustments`——决赛赛果已入库（07-19 Spain 1-0 Argentina、07-18 季军赛 France 4-6 England，load_raw 实测），freeze 对已完赛场次永不再写（正确不变量），测试「回拨 now_bj 冻决赛」前提失效。改判：剔除 `date >= 2026-07-19` 赛果行复现赛前场景，机制断言全保留（红线 4 记录见 progress.md 07-23 条）。**141 全绿，生产零改动**。
- **下一步**：按收官任务书推进——① 阶段 A 收口核查（A3 升班马/映射逐项实测确认；A4 勘误段已在位）；② 决赛已回补 → 观测 wc2026 归档模式是否自然激活、生产实例是否已重启新代码；③ 然后入阶段 B（teams 统一表，B1 保险丝：动世界杯账本结构需暂停等拍板）。

## 📌 上一次接手（2026-07-19 深夜 · 一致性大修：半衰期统一/官方同分规则/缓存新鲜度/伤停门控/口径审计，116→141 测试绿）
- **P0×5 确定性错误全清（详见 CHANGELOG 2026-07-19 条）**：
  ① **半衰期统一到 `config.NATIONAL_HALF_LIFE=730` 单一配置源**——修掉 simulate.py 硬编码 240、data.py/backtest.py 默认 547、**test_core fixture 的 240（此前每跑一次测试就把共享 model.pkl 覆盖成非生产版本，是漂移事故真源）**；get_model 仅生产参数可写共享缓存 + 测试禁旧字面量再现。
  ② **小组同分 = FIFA 2026 官方规则**（新规相互战绩优先/递归重算/不抽签/终极 FIFA 排名）——共用模块 **`tiebreak.py`** 三路统一（向量化 MC/simulate_once/project）；官方 2026-06-11 版 FIFA 排名 48 队入库 `data/fifa_rankings_2026_06.json`；纪律分无数据=可审计降级留痕（`tb_audit`/`tiebreak_audit` 输出），官方标准穷尽才允许随机；495 映射回归测试在位。
  ③ **缓存新鲜度**：get_model 数据指纹（results.csv+live_results.json mtime+size）+ `model_meta.json`（trained_through/场次/时间），数据更新自动重训、原子写、损坏自恢复；app._refit_all 走 save_model_cache 同步 meta。
  ④ **伤停默认纯 DC**：availability 登记须 verified+TTL(7d) 内新鲜或 ESPN 首发比赛级确认才生效；app 需 `WC_INJURIES=1`、simulate 需 `--injuries` 显式启用；冻结账本每条记录 adjustments；2026-06-08 Kimi 种子数据被门控为零影响（留痕不删）。
  ⑤ **90分钟/含加时口径**（`docs/score-basis.md`）：`data.known_et_mask` 识别 714 场已知加时（46 场 90' 标签错）；A/B 回测剔除反而 ΔRPS +0.00023 → 维持全量、记为限制；explainer 措辞改诚实近似。
- **P1×4**：⑥ `bt_wc.py` 世界杯专项分层回测——**WC 正赛合并 RPS .1864/命中 61.0%（经典 64 队各届仅 53-59%、RPS≈.20）；2026 同场闭盘胜模型 Δ+.0052 CI[−.0059,+.0168]**；混合口径 59.7% 禁止再当 WC 能力引用（README 三语已改）。⑦ 欠离散诊断复现（离散比 1.17-1.21）、NB2 α=0.1 修尾部但 RPS 劣化 → **不采纳保留纯 DC**，UI 比分板+README 明示「胜平负比精确比分更可靠；尾部偏瘦」。⑧ bayes 收敛诊断恢复（R-hat/ESS/divergence，不达标 exit 2 不发布、champ_ci 拒读未收敛 npz）、伪后验口径明示、MC/参数方差分解（mc_sd/param_sd，实测 10.7pp vs 1.5pp）、UI 改「90% 区间带（伪后验）」；ρ 混用声明+测试锁定。⑨ 淘汰赛晋级三路分解 `advancement_paths`（90'胜+加时 xG×⅓ 泊松+点球 50% 先验；实证 572 场点球强队仅赢 53.5%±4.1%），`docs/knockout-approx.md`。
- **P2**：两份 SKILL.md（~/.claude + ~/.agents）升 v3.1（AGENTS.md→CLAUDE.md、测试数动态化、anaconda 路径、开盘赔率现状、本轮修复档案）；README 三语修 84 项测试数+命中率口径；CHANGELOG 详录。
- **⚠️ 生产实例未重启**：决赛（北京 7-20 03:00）前不动运行中 launchd 守护（旧代码仍在跑）；**决赛回补后重启即生效全部修复**。⚠️ simulate 输出会随修复变化（同分规则/点球先验/无 jitter），旧 seed 结果不可比对是预期行为。
- **Python 一律 `/opt/anaconda3/bin/python3`**（Homebrew python3 缺 pandas）。

## 📌 上一次接手（2026-07-19 晚四 · 数据库拓展裁决落档 + 25-26 整季回补重训，116 测试绿）
