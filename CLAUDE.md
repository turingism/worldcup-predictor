...
- **净胜球校准基线（`bt_calib_margin.py`，只量不改）**：264 场 WC 正赛 as-of 样本外（2014/18/22/26G，half_life=730）。**诊断一致：模型欠离散/过度自信**——含加时 var(z)=1.187>1、**平局格 d=0 预测 26.3% vs 实际 23.1%(+3.2pp，最狠)**、**大比分尾 |d|≥3 预测 14.8% vs 实际 17.8%(−3.0pp)**、让球 cover 全档校准差 −2~−3pp（强队实际打出更多）。correct-score LogLoss=2.964。90 分钟子集同样欠离散。**根因疑似进球分布欠离散（`nb_alpha=0`）+ DC ρ 往平局推**。**改法待用户点头且必须 backtest（候选调 `nb_alpha`，`bt_nb.py` 有骨架），现停在「已定位偏差来源」**。
